...
export default function DevisForm() {
  ...
  const [dateValidite, setDateValidite] = useState(addDaysISO(30));
  ...
  useEffect(() => {
    if (!isEdit) return;
    ipc.invoke<{ devis: { client_id: number; date_emission: string; date_validite: string; objet: string; delai_realisation: string | null; conditions_paiement: string; notes: string | null }; lignes: Ligne[] }>('devis:get', Number(id))
      .then((d) => {
        setClientId(d.devis.client_id);
        setDateEmission(d.devis.date_emission);
        setDateValidite(d.devis.date_validite);
        setObjet(d.devis.objet);
        setDelai(d.devis.delai_realisation ?? '');
        setConditions(d.devis.conditions_paiement);
        setNotes(d.devis.notes ?? '');
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (isEdit) {
        await ipc.invoke('devis:update', Number(id), payload);
      } else {
        await ipc.invoke('devis:create', payload);
      }
      navigate('/devis');
    } catch (err) {
  ...
  return (
    <div className="p-8">
      <div className="mb-6 flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight">{isEdit ? 'Modifier le devis' : 'Nouveau devis'}</h1>
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_300px]">
        <div className="space-y-6">
          <Card>
            <CardContent className="space-y-4 p-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5 col-span-2">
                  <Label>Objet *</Label>
                  <Input value={objet} onChange={(e) => setObjet(e.target.value)} placeholder="Ex : Isolation combles perdus" />
                </div>
                <div className="space-y-1.5 col-span-2">
                  <Label>Délai de réalisation</Label>
                  <Input value={delai} onChange={(e) => setDelai(e.target.value)} placeholder="Ex : 2 semaines" />
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="space-y-4 p-6">
              ...
              {lignes.length === 0 ? (
                <div className="rounded-md border border-dashed p-8 text-center text-sm text-muted-foreground">
                  Aucune ligne. Ajoutez depuis le catalogue ou créez-en une personnalisée.
                </div>
              ) : (
          ...
          <Card>
            <CardContent className="space-y-4 p-6">
              <h3 className="text-base font-semibold">Conditions & notes</h3>
              <div className="space-y-1.5">
              ...
              <div className="space-y-1.5">
                <Label>Notes</Label>
                <Textarea rows={3} value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Notes internes ou visibles en bas du PDF" />
              </div>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="space-y-4">
          ...
          <Button onClick={submit} disabled={saving} className="w-full">
            ...
            {saving ? 'Enregistrement…' : isEdit ? 'Enregistrer les modifications' : 'Enregistrer (brouillon)'}
          </Button>
          <Button variant="outline" onClick={() => navigate('/devis')} className="w-full">
            Annuler
          </Button>
        </div>
      </div>
